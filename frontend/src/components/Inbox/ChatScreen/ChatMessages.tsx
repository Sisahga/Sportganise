...
import { format, isToday, isThisWeek, parseISO, differenceInMinutes } from "date-fns";
import {toZonedTime } from "date-fns-tz";

const ChatMessages = ({ messages }: ChatMessageProps) => {
  const userId = 2; // TODO: Take cookie user id.

  const formatSentAt = (sentAt: string, timeZone: string = "America/New_York") => {
        ...
        {messages.map((message, index) => {
          ...
          return (
            <div key={message.messageId} className="mb-4">
              ...
              <div className={`flex items-end gap-2 ${message.senderId == userId ? "justify-end" : ""}`}
              >
                {/* Sender Avatar */}
                ...
                >
                  {
                    message.senderId !== userId && (
                          <div className="px-3">
                            <p className="text-xs font-extralight">{message.senderFirstName}</p>
                          </div>
                    )
                  }
                  {/* Message Content */}
