...
export default function DeleteRequest({
  ...
  const handleApproveStatusChange =
      async (status: DeleteChannelRequestMemberStatus) => {
        const setApproverDeleteStatusDto: SetDeleteApproverStatusDto = {
          deleteRequestId: deleteRequest?.deleteChannelRequestDto.deleteRequestId || 0,
        ...
        if (response.statusCode === 303) {
          log.info("Channel {} deleted.", deleteRequest?.deleteChannelRequestDto.channelId);
          toast({
        ...
        } else if (response.statusCode === 204) { // 204 if delete request was denied.
          log.info("Channel {} request to delete denied.", deleteRequest?.deleteChannelRequestDto.channelId);
          toast({
          ...
          const messagePayload: SendMessageComponent = {
            ...
            channelId: deleteRequest?.deleteChannelRequestDto.channelId || 0,
            messageContent:
                `DELETE*${currentUserId}*You ${denyMessage}*${cookies.firstName} ${denyMessage}`,
            attachments: [],
        ...
        } else if (response.statusCode === 200) { // 200 if status was successfully set without altering channel state.
          log.info("Account {} approved to delete the channel, waiting on other members.", currentUserId);
          ...
          log.error("Error setting approval status for channel {}: {}",
              deleteRequest?.deleteChannelRequestDto.channelId, response.message);
          toast({
            title: "Error: ",
  ...
  const [isDeleteRequestDrawerOpen, setIsDeleteRequestDrawerOpen] =
      useState<boolean>(deleteRequestActive && currentUserApproverStatus == DeleteChannelRequestMemberStatus.PENDING);

  return (
      <Drawer open={isDeleteRequestDrawerOpen} onOpenChange={setIsDeleteRequestDrawerOpen} direction="top">
          ...
          <div>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <CircleAlert style={{color: "var(--primary-red)"}} strokeWidth={3}/>
              </div>
        ...
        <DrawerContent className="fixed top-0 left-0 right-0 min-h-64 rounded-b-[10px] translate-y-0">
          <div className="mx-auto w-full max-w-sm">
            <DrawerHeader>
              <div className="absolute top-8 left-1/2 -translate-x-1/2"><TriangleAlert
                  style={{color: "var(--primary-red)"}}/></div>
              ...
              >
                <span className="flex flex-col gap-2">
                  <span>
                    <strong>
                      {deleteRequest?.deleteChannelRequestDto.creatorId === currentUserId
                          ? "You "
                  ...
                  <span
                      className="primary-red font-semibold">
                    All contents of the channel will be permanently deleted when approved.
                  </span>
                </span>
              </DrawerDescription>
            </DrawerHeader>
                  ...
                  >
                    <span className="text-sm">
                      ...
                      <Badge
                          ...
                          variant="secondary"
                          className={`${
                                        member.status === DeleteChannelRequestMemberStatus.APPROVED
                      ...
                      >
                        {member.status === DeleteChannelRequestMemberStatus.APPROVED && "Approved"}
                        {member.status === DeleteChannelRequestMemberStatus.PENDING && "Pending"}
                      </Badge>
                  </div>
              ))}
            </div>
                ...
                {deleteRequest?.deleteChannelRequestDto.creatorId === currentUserId
                    ...
                    (
                        <Button
                            variant="default"
                            className="flex-1"
                            onClick={() => handleApproveStatusChange(DeleteChannelRequestMemberStatus.DENIED)}
                        >
                          Cancel Request
                        </Button>
                    )
                    ...
                    : (
                        <>
                          <Button
                              ...
                              onClick={() => handleApproveStatusChange(DeleteChannelRequestMemberStatus.APPROVED)}
                          >
                            Approve
                          </Button>
                          <Button
