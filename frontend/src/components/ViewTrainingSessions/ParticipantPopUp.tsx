...
const ParticipantPopUp: React.FC<ParticipantPopUpProps> = ({ accountId, isOpen, onClose }) => {
  const { data: accountDetails, loading, error } = usePersonalInformation(accountId);
  ...
  const handleSendMessage = async () => {
    ...
    const channelResponse = await createChannel(newChannelDetails, currentUserId);
    if (channelResponse?.statusCode === 201 || channelResponse?.statusCode === 302) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            {loading ? (
              ...
              <p className="text-red-500 text-sm font-normal">Failed to load account details</p>
            ) : (
              <>
                <h4 className="text-lg font-semibold">{accountDetails?.firstName} {accountDetails?.lastName}</h4>
