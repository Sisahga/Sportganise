...
public class DirectMessageChannelMemberController {
  ...
  public DirectMessageChannelMemberController(DirectMessageChannelMemberService directMessageChannelMemberService) {
  ...
  public ResponseEntity<Void> markChannelAsRead(@PathVariable int channelId, @PathVariable int accountId) {
    int updatedRows = this.directMessageChannelMemberService.markChannelAsRead(channelId, accountId);
    if (updatedRows == 0) {
      log.error("Failed to mark channel as read for channel id: {} and account id: {}", channelId, accountId);
      return ResponseEntity.notFound().build();
    }
    log.info("Successfully marked channel as read for channel id: {} and account id: {}", channelId, accountId);
