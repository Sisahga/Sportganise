...
public interface ProgramRecurrenceRepository extends JpaRepository<ProgramRecurrence, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM ProgramRecurrence rp WHERE rp.programId = :programId AND rp.occurrenceDate <= :expiryDate")
    void deleteExpiredRecurrences(@Param("expiryDate") ZonedDateTime expiryDate, @Param("programId") Integer programId);
