...
public class VerificationServiceTest {
    ...
    public void calculateExpiryDate_shouldReturnCorrectExpiryTime() {
        Timestamp expectedExpiryDate = Timestamp.valueOf(LocalDateTime.now().plusMinutes(10).minusSeconds(2));
        ...
        assertTrue(expiryDate.before(Timestamp.valueOf(LocalDateTime.now().plusMinutes(10).plusSeconds(2))), "Expiry date should be within a reasonable range.");
    }

    @Test
    public void createVerification_shouldSaveVerification() {
        Account mockAccount = new Account();
        Verification mockVerification = new Verification(mockAccount, 123456, Timestamp.valueOf(LocalDateTime.now().plusMinutes(10)));
        ...
        verify(mockVerificationRepository, times(1)).save(any(Verification.class));  // Ensure save is called once
    }
